...
function getHowItWorks(slug: string): string[] {
  const map: Record<string, string[]> = {
    ...
    'contains-duplicate': [
      'This is NeetCode 150’s opening problem — the canonical "do you reach for a hash set" interview check.',
      'Create an empty hash set called seen.',
      'Walk the array once; for each value, check membership in seen BEFORE inserting anything.',
      'If the value is already in seen, a duplicate exists — return true immediately (short-circuit, don’t scan the rest).',
      'If the loop finishes with no hit, every value was unique — return false.',
      'The brute-force O(n²) nested-loop or the O(n log n) sort-then-scan are both valid fallbacks to mention, but the hash-set pass is the expected optimal answer.',
    ],
    'valid-anagram': [
      'Two strings are anagrams only if they have equal length and identical character frequencies — check the length first as a cheap early exit.',
      'Allocate a fixed-size array of 26 zeros (assuming lowercase a–z; use a hash map instead for full Unicode).',
      'Walk string s, incrementing count[ch - \'a\'] for every character.',
      'Walk string t, decrementing the same slots.',
      'If s and t are anagrams, every slot nets back to exactly zero — check that at the end.',
      'This beats the sort-and-compare approach (O(n log n)) by doing the whole check in one O(n) pass with O(1) auxiliary space.',
    ],
    'two-sum': [
      'The single most-asked interview question on LeetCode — know the O(n) hash-map solution cold, not just the O(n²) brute force.',
      'Create an empty hash map from value → index called seen.',
      'For each index i with value num, compute complement = target - num.',
      'Check if complement already exists in seen — if so, you’ve found your pair; return [seen[complement], i] immediately.',
      'Otherwise, record seen[num] = i and keep scanning.',
      'This is a single pass: you never need a second loop, because by the time you’d need a partner, you’ve already recorded everything that came before it.',
    ],
    'group-anagrams': [
      'Anagrams share a common "signature" — sort each string’s characters and identical anagrams collapse to the same key.',
      'Create a hash map from signature → list of original strings.',
      'For each string, compute its sorted-character key (or a 26-length character-count tuple for an O(n·k) alternative that avoids the log k sort).',
      'Append the original string to groups[key], creating the list on first use.',
      'Return the map’s values as the final grouped lists.',
      'The count-array key is the more optimal variant — sorting each string costs O(k log k), while counting costs only O(k).',
    ],
    'top-k-frequent-elements': [
      'A naive sort-by-frequency approach costs O(n log n); bucket sort by frequency gets this down to O(n).',
      'Count every value’s frequency in a hash map (one O(n) pass).',
      'Create n+1 buckets (index 0..n) — frequency can never exceed n, so this bound is safe.',
      'Place each distinct value into buckets[frequency].',
      'Scan buckets from index n down to 1 (highest frequency first), collecting values until you have k of them.',
      'This is the classic follow-up to "can you do better than sorting?" — a min-heap of size k is the O(n log k) alternative worth mentioning.',
    ],
    'product-of-array-except-self': [
      'The constraint "no division" rules out the naive total-product-then-divide-by-self approach (which also breaks on zeros).',
      'Make a first pass left-to-right: res[i] accumulates the running PREFIX product of everything before index i.',
      'Make a second pass right-to-left: multiply res[i] by a running SUFFIX product of everything after index i.',
      'After both passes, res[i] = (product of everything left of i) × (product of everything right of i) — exactly the answer, in O(n) time.',
      'Only two scalar accumulators (prefix, suffix) are needed beyond the output array — O(1) auxiliary space.',
      'A very common whiteboard follow-up: what if the array contains a zero, or two zeros? Trace through it — this prefix/suffix method handles both correctly without special-casing.',
    ],
    'valid-sudoku': [
      'You only need to validate the CURRENT board’s rules — this is not a Sudoku solver, just a checker.',
      'Keep three families of "seen digit" sets: one per row (9), one per column (9), one per 3x3 box (9).',
      'The box index for cell (r, c) is (r // 3) * 3 + (c // 3) — memorize this formula, it comes up in every grid-partitioning problem.',
      'Scan all 81 cells once; skip \'.\' (empty) cells.',
      'For a filled cell, check whether its digit is already in that row’s, column’s, or box’s set — if so, return false immediately.',
      'Otherwise add the digit to all three sets and continue; if the scan finishes clean, the board is valid. Runtime is O(1) because the board size is fixed at 9x9.',
    ],
    'encode-and-decode-strings': [
      'A naive delimiter like a comma or pipe breaks the moment a string in the list contains that character — you need a delimiter-free scheme.',
      'Encode each string with a length prefix: "{length}#{string}", then concatenate all of them.',
      'Because the reader knows exactly how many characters to consume once it reads the length, the string’s own content (even \'#\' or digits) can never be misread as a delimiter.',
      'To decode: scan for the next \'#\', parse everything before it as an integer length, then slice exactly that many characters as the next string.',
      'Advance your cursor past the extracted string and repeat until the whole encoded string is consumed.',
      'This is the same length-prefix framing used by real wire protocols (e.g., Redis’s RESP bulk strings, Thrift/Protobuf length-delimited fields) — not just an interview trick.',
    ],
    'longest-consecutive-sequence': [
      'The O(n log n) sort-then-scan solution is easy but not optimal — the hash-set approach hits the required O(n).',
      'Put every value into a hash set for O(1) membership checks.',
      'The key trick: only START counting a sequence from a value num where num - 1 is NOT in the set — that guarantees num is the smallest element of its run.',
      'From each valid start, count upward (num+1, num+2, …) while each next value is present in the set, tracking the run length.',
      'Track the maximum run length seen across all starts — that’s the answer.',
      'Because every number is only ever extended from its true sequence head, each number is visited O(1) amortized times total, giving true O(n), not O(n²).',
    ],
  };
...
function getUseCases(slug: string): string {
  const map: Record<string, string> = {
    ...
    'contains-duplicate': "A NeetCode 150 / FAANG-interview staple used to test whether a candidate reaches for a hash set instead of nested loops. The same seen-set pattern shows up constantly in production: deduplicating event streams, detecting repeated primary keys before an insert, and cycle detection in linked lists/graphs (Floyd's or a visited-set variant).",
    'valid-anagram': "Classic NeetCode 150 warm-up for hash-based frequency counting. The same 26-slot counting technique powers spell-checkers and word-game solvers (Scrabble/Wordle letter-bag validation), plagiarism/duplicate-content detection via character histograms, and is the base case that Group Anagrams and many string-interview problems build on.",
    'two-sum': "The most-solved problem on LeetCode and often the very first question in a FAANG phone screen — interviewers use it to see whether you recognize the O(n) hash-map pattern immediately. The complement-lookup idea generalizes directly to 3Sum/4Sum, subarray-sum-equals-k, and to real systems like matching buy/sell orders in a trading engine or finding pairs of transactions that net to zero for fraud checks.",
    'group-anagrams': "A frequent NeetCode 150 / mid-level interview question testing hash-map-of-lists design. Anagram-signature grouping is used in real word-game engines (finding all valid anagrams of a rack in Scrabble solvers) and in bioinformatics for clustering DNA/RNA fragments with identical composition but different orderings.",
    'top-k-frequent-elements': "A go-to interview question for bucket sort vs. heap trade-offs. Directly mirrors real systems: trending-hashtag / 'most played songs' features (Twitter, Spotify), cache eviction policies like LFU (Least Frequently Used) that need the least-frequent items fast, and autocomplete ranking by query frequency.",
    'product-of-array-except-self': "A favorite FAANG interview question specifically because it forbids the 'obvious' division-based solution, forcing candidates to reason about prefix/suffix passes. The same prefix-times-suffix decomposition appears in computing cumulative distributions, and is structurally identical to how compilers evaluate certain dataflow problems where a node's value depends on everything before and after it.",
    'valid-sudoku': "A well-known NeetCode 150 problem for practicing multi-constraint hash-set validation (row + column + box simultaneously) and the (r//3)*3+(c//3) box-index trick, which reappears in any grid-partitioning problem (image tiling, chessboard region checks). Real Sudoku solver apps use this exact validity check as their innermost constraint-satisfaction step before attempting backtracking search.",
    'encode-and-decode-strings': "A NeetCode 150 / Google-style interview question testing serialization design without relying on a 'convenient' delimiter. Length-prefix framing is not just a trick — it's exactly how real wire protocols work: Redis's RESP protocol frames bulk strings as \"$<length>\\r\\n<data>\\r\\n\", and Thrift/Protobuf/gRPC all use length-delimited fields so payload bytes can never be confused with framing bytes.",
    'longest-consecutive-sequence': "A classic 'can you beat the obvious O(n log n) sort' NeetCode 150 problem. The sequence-head trick (only start counting where num-1 is absent) is a reusable pattern for any 'find runs/streaks in unsorted data' problem, and the same hash-set-membership idea underlies range-compression and interval-merging techniques used in log analysis and time-series gap detection.",
  };
