...
`localStorage` key stayed `dsaq.theme` (unchanged at the time) so existing users' saved preference carried over. **Update 2026-09-19 (later same day, DSAQuest→Akgoflick rename — see [[brand-rename-algoflick]]):** this key was since renamed to `akgoflick.theme`, matching the live K-spelled brand (`Akgoflick`/`AKGO`) — verified consistent as of the category-icons task later that same day, no leftover mismatch. Same rename touched the other store-owned keys (`vizPanelOpen`, `vizPanelWidth`, `codeEditorOpen`). Anyone whose `dsaq.theme`/`dsaq.*` was set pre-rename silently resets to the default theme once, since the key changed. The `data-theme` attribute is applied **inside the store module**, both at module load (`applyThemeAttr(initialTheme)` runs synchronously when the store file is imported, before first paint — avoids FOUC) and inside `toggleTheme`/`setTheme`. `App.tsx` now just reads `theme`/`toggleTheme` via selectors and passes them to `Navbar` — it does no theme logic itself anymore.
...
See also [[project_dsa_platform]], [[brand-rename-algoflick]].
